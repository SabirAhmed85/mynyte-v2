...
export class OfferService {
  public offers$: BehaviorSubject<OfferListing[]> = new BehaviorSubject<OfferListing[]>(offers);

  public getOffers(): BehaviorSubject<OfferListing[]> {
    return this.offers$;
  }

  public getOffersForBusiness(businessId: number): Observable<OfferListing[]> {
    return this.offers$.pipe(map(results => results.filter(offer => offer.createdByBusinessId === businessId)));
  }

  public getOffer(id: number): Observable<OfferListing> {
